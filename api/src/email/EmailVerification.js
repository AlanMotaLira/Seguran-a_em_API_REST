...
class EmailVerification extends Email {
  constructor(user, address) {
    ...
    (this.text = `Óla! Este é um email de verificação do Blog, enviado para ${user.name}! Verifique seu email aqui:${address}`),
      (this.html = `
        <h1>Olá!</h1>
        <p>Este é um email de verificação do Blog!</p>
        <p>Enviado para ${user.name}!</p>
        <p>Verifique seu email aqui:<a href="${address}">${address}</a></p>
        `);
  }
}
